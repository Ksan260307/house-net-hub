...
| 機能 | 説明 |
|------|------|
| 📶 **WiFi QRコード生成** | SSID・パスワード等から即座にQRを生成。**PNG／SVG保存・来客用カード印刷**に対応 |
| 🖥 **来客用フルスクリーン表示** | QRを大きく表示。玄関先やリビングのタブレットでそのまま提示できます |
| 📇 **ネットワークプロファイル管理** | 保存・**編集**・削除・来客用マーク。**JSONで書き出し/読み込み**（バックアップ・引っ越し） |
| 🔐 **パスワード強度診断＆生成** | 強度をその場で診断／ランダム生成に加え、口頭で伝えやすい**ことばフレーズ生成**（例: sakura-228-yama-tori） |
| 🚀 **かんたん通信速度テスト** | 下り・**上り**・応答(ping)・**ジッター**を計測。**履歴を暗号化保存**しグラフ表示（日時ツールチップ付き） |
| 🩺 **かんたんネット診断** | 質問に答えるだけで原因と対処法を案内（接続不可／低速／特定機器／特定サイト） |
| 🧮 **データ使用量シミュレーター** | 1日の利用時間から月間データ使用量の目安を計算 |
| 🎨 **おえかき3D**（子供向け） | 描いた絵を立体化し、海岸ステージの街に置いていくミニゲーム |
| 🌙 **ダークモード** | ヘッダーのボタンで切替（設定保存・OS設定に追従）。オンライン/オフライン表示付き |

#### 🎨 おえかき3D について
- **おえかき／ステージのサブタブ構成**。描いて「3Dにする！」を押すと自動でステージへ。ステージは**全画面表示**にも対応。
- おえかきツール: **ふで／ぬりつぶし／けしゴム**、**もどす・すすむ（Undo/Redo）**、**したがきガイド**（🐟🦀🏠🐦の点線テンプレート。完成品には写らない）、**12色＋すきないろ（自由カラーピッカー）**、太さ4段階。
- **リアルタイム3Dプレビュー**: 描いている絵が立体化されるとどう見えるか、その場でゆらゆら動くプレビューで確認できる。
- 立体化時に**ダークアウトライン**を自動付与し、砂浜の上でもパキッと見える。
- 生きものの動き: **ワドル歩行（左右ゆれ）**・**ジャンプのスカッシュ&ストレッチ**・**着地/登場/退場の砂ぼこりパーティクル**・ときどき立ち止まる休憩。
- 空の演出: **流れる雲**と**ときどき飛んでくる鳥**。
- **効果音**（Web Audio合成・外部ファイル不要）: 登場ポン・ジャンプboing・着地・ばいばい。🔊ボタンでミュート（設定は保存）。
- **パン（スクロール）**: なにもない所をドラッグで視点移動。**2本指ピンチでズーム**（iPad対応）。小さいなかまもタップしやすい当たり判定。
- **街ランク**: なかまが増えると むら → まち → にぎやかなまち → だいとかい に成長。
